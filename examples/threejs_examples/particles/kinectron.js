...
function initKinectron() {
	...
	kinectron1.startMultiFrame(["raw-depth", "body"], mfCallback);
...
function mfCallback(dataRcvd) {
	if (dataRcvd.body) {
		drawJoints1(dataRcvd.body);
		drawJoints2(dataRcvd.body);
	} 
}
...
function drawJoints1(data) {
  ...
  jointPositions1 = scaleSkeleton(data, 500);
...
function drawJoints2(data) {
	...
	// scale the joints to screen
	jointPositions2 = scaleSkeleton(data, 1000);
...
function scaleSkeleton(data, offset) {
	...
	for (var m = 0; m < data.joints.length; m++) {
		...
		data.joints[m].cameraZ = (data.joints[m].cameraZ * -window.innerWidth/4) + offset;
	}
